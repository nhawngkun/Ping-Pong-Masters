...
public class UIManager_PingPongMasters : Singleton<UIManager_PingPongMasters>
{

    [SerializeField] private List<UICanvas_PingPongMasters> uiCanvases;
    [SerializeField] private CanvasGroup ChooseAI, howToPlay, home, winPopUp, settingPanel, goButton, Loss, gamplayPanel, levelPanel, shopPanel, updatePanel;
    ...
    public void PlayUISound(int id)
    {
        if (SoundManager_PingPongMasters.Instance != null)
            SoundManager_PingPongMasters.Instance.PlayVFXSound(id);
    }
    ...
    public void EnableChooseAI(bool enable)
    {
        if (enable) ChooseAI.DOFade(1f, time).Play();
        else ChooseAI.DOFade(0f, time).Play();
        ChooseAI.blocksRaycasts = enable;
        ChooseAI.interactable = enable;
    }
    ...
    public void EnableLoss(bool enable)
    {
        if (enable)
        {
            // ✅ GỌI Open() trước khi fade
            UILoss_PingPongMasters lossUI = Loss.GetComponent<UILoss_PingPongMasters>();
            if (lossUI != null)
            {
                lossUI.Open(); // ← Gọi Open() để update text
            }
    ...
    public void EnableWin(bool enable)
{
    if (enable)
    {
        ...
        UIWin_PingPongMasters winUI = winPopUp.GetComponent<UIWin_PingPongMasters>();
        if (winUI != null)
        {
            winUI.Open(); 
        }
        ...
        {
            // Nếu script không nằm cùng object với CanvasGroup, thử tìm trong list uiCanvases
            winUI = GetUI<UIWin_PingPongMasters>();
            if (winUI != null) winUI.Open();
        }
        // ⭐⭐⭐ KẾT THÚC ĐOẠN THÊM ⭐⭐⭐

        winPopUp.DOFade(1f, time).Play();
    }
    else 
    {
    ...
    public T OpenUI<T>() where T : UICanvas_PingPongMasters
    {
        T canvas = GetUI<T>();
        if (canvas != null)
        {
    ...
    public void CloseUI<T>(float time) where T : UICanvas_PingPongMasters
    {
        T canvas = GetUI<T>();
        if (canvas != null)
        {
    ...
    public void CloseUIDirectly<T>() where T : UICanvas_PingPongMasters
    {
        T canvas = GetUI<T>();
        if (canvas != null)
        {
    ...
    public bool IsUIOpened<T>() where T : UICanvas_PingPongMasters
    {
        T canvas = GetUI<T>();
    ...
    public T GetUI<T>() where T : UICanvas_PingPongMasters
    {
